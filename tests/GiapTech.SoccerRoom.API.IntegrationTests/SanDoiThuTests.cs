...
/// <summary>
/// SÀN ĐỐI THỦ + lời mời bắt đối — chỗ RỘNG NHẤT hệ thống đọc dữ liệu ngoài tenant.
///
...
/// </summary>
public class SanDoiThuTests(ApiFactory factory) : IClassFixture<ApiFactory>
{
    ...
    private async Task<(HttpClient Client, string MaDoi)> ClbRieng(string nhan)
    {
        ...
        var dangKy = await moTai.PostAsJsonAsync("/api/v1/dang-ky-clb",
            new { TenDoi = $"Sàn {nhan} {Guid.NewGuid():N}" });
    ...
    private static async Task<List<JsonElement>> DocSan(HttpClient c, string query = "")
    {
        var body = await c.GetFromJsonAsync<JsonElement>($"/api/v1/san-doi-thu{query}");
    ...
    [Fact]
    public async Task San_liet_ke_CLB_khac_nhung_KHONG_co_chinh_minh()
    {
        var client = await Client(factory.MaDoiA);

        var ds = await DocSan(client);
        ...
        // Không ai bắt đối với chính mình; để lại chỉ gây nhầm khi bấm "Gửi lời mời".
        Assert.DoesNotContain(factory.MaDoiA, ma);
    }

    [Fact]
    public async Task San_khong_lo_du_lieu_noi_bo_cua_CLB_khac()
    {
    ...
    [Fact]
    public async Task San_khong_tra_lien_he_du_CLB_da_khai()
    {
        ...
        var ds = await DocSan(clientA, "?soDong=100&tuKhoa=" + Uri.EscapeDataString("Sàn khai-lien-he"));
    ...
    public async Task Thanh_tich_dem_dung_tran_cua_CLB_do_khong_lan_sang_CLB_khac()
    {
        ...
        var ds = await DocSan(clientC, "?soDong=100&tuKhoa=" + Uri.EscapeDataString("Sàn thanh-tich"));
    ...
    public async Task Tim_theo_ten_va_loc_theo_khu_vuc()
    {
        ...
        var theoTen = await DocSan(client, "?tuKhoa=" + Uri.EscapeDataString("Đội B"));
        ...
        var khongKhop = await DocSan(client, "?tuKhoa=khongCoDoiNaoTenNhuVay");
    ...
    {
        var client = factory.CreateClient();
        var res = await client.GetAsync("/api/v1/san-doi-thu");
    ...
    public async Task Loi_moi_hien_o_ca_hai_ben_va_KHONG_hien_o_CLB_thu_ba()
    {
        ...
        var gui = await clientA.PostAsJsonAsync("/api/v1/san-doi-thu/loi-moi", new
        {
            MaDoiNhan = maB,
        ...
        // A thấy: ta gửi.
        var cuaA = await clientA.GetFromJsonAsync<JsonElement>("/api/v1/san-doi-thu/loi-moi");
        ...
        // B thấy: ta nhận.
        var cuaB = await clientB.GetFromJsonAsync<JsonElement>("/api/v1/san-doi-thu/loi-moi");
        ...
        // C KHÔNG thấy gì cả.
        var cuaC = await clientC.GetFromJsonAsync<JsonElement>("/api/v1/san-doi-thu/loi-moi");
    ...
    public async Task CLB_thu_ba_khong_tra_loi_duoc_loi_moi_cua_nguoi_khac()
    {
        ...
        var gui = await clientA.PostAsJsonAsync("/api/v1/san-doi-thu/loi-moi",
        ...
        var traLoi = await clientC.PostAsJsonAsync(
            $"/api/v1/san-doi-thu/loi-moi/{id}/tra-loi", new { ChapNhan = true, PhanHoi = (string?)null });
        Assert.Equal(HttpStatusCode.NotFound, traLoi.StatusCode);

        // Cả huỷ cũng không được — huỷ chỉ dành cho bên GỬI.
        var huy = await clientC.DeleteAsync($"/api/v1/san-doi-thu/loi-moi/{id}");
    ...
    public async Task Chap_nhan_tao_tran_o_CA_HAI_lich_doc_lap()
    {
        ...
        var traLoi = await clientB.PostAsJsonAsync($"/api/v1/san-doi-thu/loi-moi/{id}/tra-loi",
    ...
    public async Task Khong_gui_trung_loi_moi_dang_cho()
    {
        ...
        var lan1 = await clientA.PostAsJsonAsync("/api/v1/san-doi-thu/loi-moi",
            new { MaDoiNhan = maKia, LoiNhan = "lần 1" });
        lan1.EnsureSuccessStatusCode();

        // Bấm hai lần không được dội hai thẻ vào hòm thư bên kia.
        var lan2 = await clientA.PostAsJsonAsync("/api/v1/san-doi-thu/loi-moi",
    ...
    public async Task Khong_tu_moi_chinh_minh()
    {
        ...
        var res = await clientA.PostAsJsonAsync("/api/v1/san-doi-thu/loi-moi",
    ...
    public async Task Lien_he_chi_hien_sau_khi_chap_nhan()
    {
        ...
        // CLB trên sàn để thu số. Chỉ hiện sau khi bên kia đồng ý.
        ...
        var truoc = await clientA.GetFromJsonAsync<JsonElement>("/api/v1/san-doi-thu/loi-moi");
        ...
        await clientB.PostAsJsonAsync($"/api/v1/san-doi-thu/loi-moi/{id}/tra-loi",
            new { ChapNhan = true, PhanHoi = (string?)null });

        // Sau khi đồng ý: thấy liên hệ để hẹn nhau.
        var sau = await clientA.GetFromJsonAsync<JsonElement>("/api/v1/san-doi-thu/loi-moi");
    ...
    public async Task Tra_loi_hai_lan_bi_chan()
    {
        ...
        var lan1 = await clientB.PostAsJsonAsync($"/api/v1/san-doi-thu/loi-moi/{id}/tra-loi",
        ...
        var lan2 = await clientB.PostAsJsonAsync($"/api/v1/san-doi-thu/loi-moi/{id}/tra-loi",
    ...
    public async Task Ben_gui_huy_duoc_loi_moi_chua_tra_loi()
    {
        ...
        var huy = await clientA.DeleteAsync($"/api/v1/san-doi-thu/loi-moi/{id}");
        ...
        var lai = await clientA.PostAsJsonAsync("/api/v1/san-doi-thu/loi-moi",
