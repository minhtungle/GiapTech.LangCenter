...
| N11 | `globalTeardown` cho E2E — **nguyên nhân vẫn chưa chữa**. Dọn tay hai lần: 12/09/2026 (212 → 1 tenant) và **17/09/2026** (662 → 1 tenant, 51 → **12 MB**, xoá kèm 80 thư mục ảnh MinIO mồ côi 2,6 MB). Tái phát đúng như đã ghi: mỗi lượt chạy cả bộ sinh ~40 tenant. Nay có `scripts/don-tenant-e2e.sql` để dọn lại trong một lệnh, nhưng **dọn tay không phải cách chữa** — teardown mới là. Chốt 12/09 vẫn giữ: Claude kiểm chứng **trực tiếp trên `W686AE9`** | Trung bình |
