...
[đặc tả FR-15](../06-nghiep-vu/thong-ke.md):
...
phải đồng bộ giữa ba DB nếu tách. Quyết định giữ một source: [ADR-0005](../02-kien-truc/adr/0005-mot-source-va-doi-ten-langcenter.md).
...
[trien-khai-pull-code.md](../07-ha-tang/trien-khai-pull-code.md).
...
| N3 | ~~`/dang-ky-trung-tam` mở ở mọi môi trường~~ — **ĐÃ GIẢI QUYẾT 23/09/2026** bằng [ADR-0009](../02-kien-truc/adr/0009-tai-khoan-cap-he-thong.md): tạo trung tâm nay đi qua site chủ hệ thống (`POST /chu-he-thong/trung-tam`), gác bằng `[ChiChuHeThong]` — cần tài khoản chủ, không phải ai cũng gọi được. Endpoint ẩn danh cũ vẫn còn nhưng **tắt mặc định** (`CHO_TU_DANG_KY=false` từ 22/09) và chỉ bật cho E2E. Captcha/xác thực email **không cần nữa với điều kiện `CHO_TU_DANG_KY` không bao giờ bật trên VPS** — bật nó là mở lại đúng lỗ hổng cũ, và không có gì trong mã ngăn việc đó ngoài kỷ luật vận hành. Nếu sau này thật sự cần cho khách tự đăng ký thì phải làm captcha trước | ~~Cao~~ Xong (có điều kiện) |
...
| N11 | ~~`globalTeardown` cho E2E~~ — **ĐÃ CHỮA 23/09/2026**. `frontend/e2e/don-dep.ts` gọi `POST /chu-he-thong/don-tenant-e2e` sau mỗi lượt chạy. Bốn chốt chặn: chỉ xoá tên bắt đầu `E2E `, cờ `CHO_DON_E2E` mặc định TẮT (tắt ⇒ 404), gác `[ChiChuHeThong]`, trả số lượng đã xoá. Teardown thất bại **chỉ in cảnh báo**, không làm bộ test đỏ — "rác chưa dọn" mà bị hiểu thành "test đỏ" thì lần sau người ta tắt teardown đi. Trước đó phải dọn tay **năm lần** (12/09: 212→1, 17/09: 662→1, ba lần ngày 23/09) và repo tích 5 script `.sql` dọn rác | ~~Trung bình~~ Xong |
...
| N28 | **`thong-ke-nhan-su.spec.ts` sát hạn mức thời gian** — chạy riêng mất **27,4s** trên `timeout: 30_000`, nên trong bộ đầy đủ (máy bận hơn) nó **đỏ ngẫu nhiên**: đã thấy 23/09/2026 khi chạy 63 test, chạy lại riêng thì xanh. Không phải lỗi logic. Cách chữa: tách test thành hai (tạo tiêu chí / chấm điểm), hoặc dựng dữ liệu qua API thay vì qua UI — chứ **đừng nâng timeout**, nâng chỉ giấu việc test đang làm quá nhiều việc | Thấp |
