...
| N5 | **E2E để lại tenant rác trong DB dev** | ~2h | Tính đến 20/08: **67 CLB rác / 74 tổng** — chỉ 7 CLB là dữ liệu mẫu thật. Từ 18/08 chúng **hiện trên Cộng đồng của mọi người**. Mỗi lần chạy E2E lại sinh thêm ~1 CLB/test. Hai hướng: DB riêng cho E2E, hoặc bước dọn sau khi chạy |
| N7 | **Thiếu `EnableRetryOnFailure` cho Npgsql** | ~15p | Phát hiện 20/08 khi restart container postgres: API giữ pool trỏ tới container cũ, **request đầu tiên sau đó trả 500** `LOI_HE_THONG` ("transient failure"), các request sau tự lành. Trên production nghĩa là mỗi lần DB restart/failover thì người dùng đầu tiên thấy "Đã có lỗi xảy ra". Sửa ở `Infrastructure/DependencyInjection.cs:27` — `UseNpgsql(cs, o => o.EnableRetryOnFailure())`. Lưu ý: retry không tương thích với transaction do người dùng tự mở, cần rà lại chỗ nào dùng `BeginTransaction` |
