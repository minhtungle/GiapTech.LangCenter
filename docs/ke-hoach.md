...
| 385 test (57 unit + 328 integration) | ✅ |
...
| N9 | 🔴 **Tự xoá hết quyền của chính mình được** | ~30p | Phát hiện 21/08 khi viết test cho tính năng nghỉ thi đấu: `PUT /tai-khoan/{id}` với `QuyenIds = []` cho phép admin **tự cắt hết quyền của mình**, và mọi request sau đó trả 403 — CLB mất đường quản trị nếu đó là admin duy nhất. Cùng loại rủi ro với "tự khoá tài khoản chính mình" đã chặn ở `ChoNghiThiDauHandler`. Sửa: chặn ở handler khi `Id == currentUser.UserId` mà danh sách quyền mới không còn quyền quản trị. Chưa làm vì ngoài phạm vi yêu cầu hiện tại |
