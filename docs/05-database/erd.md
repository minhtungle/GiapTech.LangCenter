...
> Cách triển khai: [../backend/multi-tenant.md](../03-backend/multi-tenant.md).
...
`TAI_KHOAN` là cách họ đăng nhập — xem [FR-03](../06-nghiep-vu/quan-tri-he-thong.md#fr-03--người-dùng-hồ-sơ-con-người).
...
| `UNIQUE(username)` | `QUAN_TRI_HE_THONG` | **Toàn cục**, khác `TAI_KHOAN` (chỉ duy nhất trong tenant): tài khoản chủ đứng trên mọi tenant nên không có phạm vi nào để thu hẹp — [ADR-0009](../02-kien-truc/adr/0009-tai-khoan-cap-he-thong.md) |
...
- Nghiệp vụ dùng các bảng này: [../nghiep-vu/](../06-nghiep-vu/README.md)
- Cách áp Global Query Filter: [../backend/multi-tenant.md](../03-backend/multi-tenant.md)
