...
Luật phụ thuộc giữa các lớp: [../backend/clean-architecture.md](../03-backend/clean-architecture.md).

## Ba cơ chế xuyên suốt

| Cơ chế | Cách hoạt động | Chi tiết |
|---|---|---|
| **Multi-tenant** | Đăng nhập nhập ID đội → middleware resolve `tenant_id` → JWT claim → EF Core `HasQueryFilter` áp dụng tự động cho mọi entity nghiệp vụ | [multi-tenant.md](../03-backend/multi-tenant.md) |
| **Phân quyền động** | Custom `IAuthorizationHandler` đọc bảng `QUYEN_CHUC_NANG` tại runtime (có cache ngắn hạn); attribute `[RequirePermission("LichThiDau", "Sua")]` trên từng endpoint — **không** dùng role cố định | [phan-quyen-dong.md](../03-backend/phan-quyen-dong.md) |
...
|---|---|---|
| Multi-tenant qua `tenant_id` | ✅ Đã chốt | [multi-tenant.md](../03-backend/multi-tenant.md) |
...
| **Ba hệ thống con HRM · CRM · LMS** | ✅ Đã chốt (08/09/2026) | **Nhóm chức năng phân quyền, KHÔNG tách service** — một API, một DB, một lần đăng nhập. [phan-quyen-dong.md](../03-backend/phan-quyen-dong.md#ba-hệ-thống-con-hrm--crm--lms) |
| Giá trị design token (màu cụ thể) | 🕓 Chưa chốt | Chốt cùng lúc dựng style-guide — [design-tokens.md](../04-frontend/design-tokens.md) |
| `tenant_id` ở bảng con | ✅ Đã chốt | Denormalize xuống **33/34 bảng** (mọi bảng trừ `TENANT`) — [ERD](../05-database/erd.md#denormalize-tenant_id-xuống-bảng-con) |
...
Xem [../ha-tang/README.md](../07-ha-tang/README.md#sơ-đồ-triển-khai).

## Đường dẫn tài liệu liên quan

- Tổng thuật nghiệp vụ: [../tong-thuat.md](../01-tong-quan/tong-thuat.md)
- Nghiệp vụ chi tiết theo module: [../nghiep-vu/](../06-nghiep-vu/README.md)
- Mô hình dữ liệu: [../database/erd.md](../05-database/erd.md)
- Thuật ngữ dễ nhầm: [THUAT-NGU.md](../01-tong-quan/thuat-ngu.md)
