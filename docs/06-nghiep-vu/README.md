...
→ Chi tiết: [phan-quyen-dong.md](../03-backend/phan-quyen-dong.md#ba-hệ-thống-con-hrm--crm--lms)
...
1. **Cách ly tenant**: mọi truy vấn nghiệp vụ lọc theo `tenant_id` — xem
   [multi-tenant](../03-backend/multi-tenant.md).
2. **Phân quyền động**: đọc từ bảng `QUYEN_CHUC_NANG` tại runtime — xem
   [phân quyền động](../03-backend/phan-quyen-dong.md).
