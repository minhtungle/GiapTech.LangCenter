...
/**
 * Đăng ký CLB mới. Mã đội do hệ thống sinh (7 ký tự) — người dùng không tự đặt vì tên
 * dạng "FC ..." rất dễ trùng giữa các CLB phong trào.
 *
...
export default function DangKyClb() {
  ...
  const dangKy = async (e: React.FormEvent) => {
    ...
    setDangGui(true)
    try {
      const { data } = await api.post<KetQua>('/dang-ky-clb', { tenDoi })
      setKetQua(data)
    } catch (err) {
        ...
        <CardContent>
          {ketQua ? (
            <div className="flex flex-col gap-4">
              ...
              <div className="rounded-md border border-border p-3 text-sm">
                <div className="flex justify-between py-0.5">
                  <span className="text-muted-foreground">{t('thietLap.tenDoi')}</span>
                  <span className="font-medium">{ketQua.tenDoi}</span>
                </div>
                <div className="flex justify-between py-0.5">
            ...
            <form onSubmit={dangKy} className="flex flex-col gap-4">
              ...
              <Button type="submit" disabled={dangGui || !tenDoi.trim()}>
